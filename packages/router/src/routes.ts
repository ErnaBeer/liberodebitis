...
export class Routes implements ReactiveController {
  ...
  constructor(
    ...
    routes: Array<RouteConfig>,
    options?: { fallback?: BaseRouteConfig; hashMode?: boolean }
  ) {
    ;(this._host = host).addController(this)
    this.routes = [...routes]
    this.fallback = options?.fallback
    this.hashMode = options?.hashMode
  }
  ...
  async goto(pathname: string) {
    ...
    if (this.routes.length === 0 && this.fallback === undefined) {
      ...
      this._currentPathname =
        tailGroup === undefined ? pathname : pathname.substring(0, pathname.length - tailGroup.length)
    }
